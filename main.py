...
def parse_one(id: int, out_dir: Path):
    """
    Здесь ваша логика парсинга: 
    скачиваете текст, картинки, формируете meta.json и кладёте в out_dir/{id}/
    """
    article_dir = out_dir / str(id)
    article_dir.mkdir(parents=True, exist_ok=True)
    # TODO: ваши calls, BeautifulSoup, httpx и т.д.
    # сохраним пример meta.json
    meta = {
        "id": id,
    ...
    (article_dir / "meta.json").write_text(json.dumps(meta, ensure_ascii=False), encoding="utf-8")
    # а дальше кладём text и картинки...


def main(state_file: str, output_dir: str):
    state_path = Path(state_file)
    posted_ids = load_posted_ids(state_path)
    logging.info("Loaded %d posted IDs", len(posted_ids))

    # 1) Считаем все возможные ID из вашего источника (RSS/API)
    #    Здесь для примера список от 1 до 100
    ...
    if not new_ids:
        ...
        # Эта строка сообщает Actions, что новых ID = 0
        print("::set-output name=new_count::0")
        return

    logging.info("Will parse %d new articles: %s", len(new_ids), new_ids)
    out = Path(output_dir)
    for i in new_ids:
        parse_one(i, out)
