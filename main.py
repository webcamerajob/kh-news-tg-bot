...
def setup_vpn():
    """
    Настраивает WireGuard-интерфейс из переменных окружения:
      WG_PRIVATE_KEY, WG_ADDRESS, WG_PEER_PUBLIC_KEY,
      WG_ENDPOINT, WG_ALLOWED_IPS (по умолчанию 0.0.0.0/0), WG_DNS.
    """
    priv  = os.getenv("WG_PRIVATE_KEY")
    addr  = os.getenv("WG_ADDRESS")
    peer  = os.getenv("WG_PEER_PUBLIC_KEY")
    endp  = os.getenv("WG_ENDPOINT")
    allowed = os.getenv("WG_ALLOWED_IPS", "0.0.0.0/0")
    dns     = os.getenv("WG_DNS", "8.8.8.8")

    if not (priv and addr and peer and endp):
        logging.info("VPN vars missing, skipping WireGuard setup")
        return
...
# Поднимаем VPN сразу при старте
setup_vpn()

def load_posted_ids(state_file: Path) -> Set[int]:
    """
    Читает articles/posted.json и возвращает set уже опубликованных ID.
    Поддерживает пустой или несуществующий файл.
    """
